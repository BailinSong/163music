...
def download_song_by_key_word(songname, singger,download_folder,sub_folder=True):
    # key_word= urllib3.quote(key_word)

    api = CloudApi()
    song_id=api.get_song_id(songname, singger)
    print(song_id)
    if song_id:
        download_song_by_id(song_id,download_folder)
